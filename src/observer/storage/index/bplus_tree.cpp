...
RC BplusTreeHandler::create(const char *file_name, bool unique, std::vector<int> attr_id, std::vector<AttrType> attr_type,
...
MemPoolItem::unique_ptr BplusTreeHandler::make_key(const char *user_key, const RID &rid)
{
  ...
  int pos = 0;
  for (int i = 0; i < file_header_.attr_num; i++) {
    memcpy(static_cast<char *>(key.get()) + pos, user_key + file_header_.attr_offset[i], file_header_.attr_length[i]);
