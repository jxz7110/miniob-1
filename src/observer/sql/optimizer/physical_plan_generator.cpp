...
RC PhysicalPlanGenerator::create_plan(InsertLogicalOperator &insert_oper, unique_ptr<PhysicalOperator> &oper)
{
  ...
  oper.reset(insert_phy_oper);//oper -> insert_phy_oper
  return RC::SUCCESS;
}
...
RC PhysicalPlanGenerator::create_plan(UpdateLogicalOperator &update_oper, std::unique_ptr<PhysicalOperator> &oper)
{
  ...
  string attribute_name = update_oper.attribute_name();
  Value &value = update_oper.value();
  oper = unique_ptr<PhysicalOperator>(new UpdatePhysicalOperator(table, std::move(attribute_name), std::move(value)));
